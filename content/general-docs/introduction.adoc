...
A list of use cases examples can be found in the https://asam-ev.github.io/OpenMATERIAL-3D/asamopenmaterial/latest/specification/use-cases/use-cases.html[Use Cases] chapter, each highlighting its relevance to the material chapter, the geometry chapter, or both.
...
More information about the file formats is given in xref:geometry/file-format-support.adoc[].
...
A detailed definition of the https://github.com/asam-ev/OpenMATERIAL-3D/blob/main/schemas/mapping_schema.json[mapping file JSON schema] is given in xref:geometry/mapping-schema.adoc[].
...
* Mapping a texture code of a dedicated {THIS_STANDARD} assignment texture to a _material property_ file, enabling detailed differentiation between _material properties_.
...
A detailed definition of the https://github.com/asam-ev/OpenMATERIAL-3D/blob/main/schemas/material_schema.json[material file JSON schema] is given in xref:material/material-schema.adoc[].
