...
Typically, these parts do not move in the simulation and therefore do not have their own transforms.
...
This group contains all parts of the vehicle's convertible top.
...
The `<door_bottom_idx>` index numbers each door on the bottom, starting from the front of the vehicle with 0 and counting toward the rear.
...
The `<door_front_idx>` index numbers all front doors, starting from the right and counting leftward in positive y-direction, beginning with 0.
...
The `<door_left_idx>` index numbers all doors on the left side, starting from the front and counting toward the rear, beginning with 0.
...
The `<door_rear_idx>` index numbers all rear doors, starting from the right and counting leftward in positive y-direction, beginning with 0.
...
The `<door_right_idx>` index numbers all doors on the right side, starting from the front and counting toward the rear, beginning with 0.
...
The `<door_top_idx>` index numbers all doors on the top of the vehicle, starting from the front and counting toward the rear, beginning with 0.
...
The `<license_plate_idx>` index numbers each license plate, counting from right to left in positive y-direction, and from front to rear, starting with 0.
...
The `<brake_center_idx>` index numbers each brake light in the center, counting from right to left in positive y-direction, starting with 0.
...
The `<brake_left_idx>` index numbers each brake light on the left side, counting from right to left in positive y-direction, starting with 0.
...
The `<brake_right_idx>` index numbers each brake light on the right side, counting from right to left in positive y-direction, starting with 0.
...
The `<corner_left_idx>` index numbers each corner light on the left side, counting from right to left in positive y-direction, starting with 0.
...
The `<corner_right_idx>` index numbers each corner light on the right side, counting from right to left in positive y-direction, starting with 0.
...
The `<day_left_idx>` index numbers each daytime running light on the left side, counting from right to left in positive y-direction, starting with 0.
...
The `<day_right_idx>` index numbers each daytime running light on the right side, counting from right to left in positive y-direction, starting with 0.
...
The `<fog_left_idx>` index numbers each fog light on the left side, counting from right to left in positive y-direction, starting with 0.
...
The `<fog_right_idx>` index numbers each fog light on the right side, counting from right to left in positive y-direction, starting with 0.
...
The `<high_beam_left_idx>` index numbers each high beam light on the left side, counting from right to left in positive y-direction, starting with 0.
...
The `<high_beam_right_idx>` index numbers each high beam light on the right side, counting from right to left in positive y-direction, starting with 0.
...
The `<indicator_left_idx>` index numbers each indicator light on the left side, counting from right to left in positive y-direction, and from the front to the rear, starting with 0.
...
The `<indicator_right_idx>` index numbers each indicator light on the right side, counting from right to left in positive y-direction, and from the front to the rear, starting with 0.
...
The `<license_plate_light_idx>` index numbers each license plate light, counting from right to left in positive y-direction, and from the front to the rear, starting with 0.
...
The `<low_beam_left_idx>` index numbers each low beam light on the left side, counting from right to left in positive y-direction, starting with 0.
...
The `<low_beam_right_idx>` index numbers each low beam light on the right side, counting from right to left in positive y-direction, starting with 0.
...
The `<park_left_idx>` index numbers each parking light on the left side, counting from right to left in positive y-direction, and from the front to the rear, starting with 0.
...
The `<park_right_idx>` index numbers each parking light on the right side, counting from right to left in positive y-direction, and from the front to the rear, starting with 0.
...
The `<position_left_idx>` index numbers each position light on the left side, counting from right to left in positive y-direction, and from the front to the rear, starting with 0.
...
The `<position_right_idx>` index numbers each position light on the right side, counting from right to left in positive y-direction, and from the front to the rear, starting with 0.
...
The `<reverse_left_idx>` index numbers each reverse light on the left side, counting from right to left in positive y-direction, starting with 0.
...
The `<reverse_right_idx>` index numbers each reverse light on the right side, counting from right to left in positive y-direction, starting with 0.
...
The `<tail_left_idx>` index numbers each tail light on the left side, counting from right to left in positive y-direction, starting with 0.
...
The `<tail_right_idx>` index numbers each tail light on the right side, counting from right to left in positive y-direction, starting with 0.
...
The `<warning_idx>` index numbers each warning light, counting from right to left in positive y-direction, and from the front to the rear, starting with 0.
...
This group contains all parts of the main moving part of a blindspot mirror, which adjusts when moved. 
It is a child of the corresponding mirror mounting.

The `<blindspot_mirror_joint_idx>` index numbers each blindspot mirror joint, counting from right to left in positive y-direction, and from the front to the rear, starting with 0.
...
It shall be a child of `<Grp_Exterior_Dynamic>` if mounted directly to the vehicle body, or a child of `<Grp_Door_Left>` or `<Grp_Door_Right>` if mounted on the door.

The `<blindspot_mirror_mounting_idx>` index numbers each blindspot mounting, counting from right to left in positive y-direction, and from the front to the rear, starting with 0.

The index is used consistently, regardless of whether the mirror is mounted on the door or the vehicle body.
...
The `<blindspot_mirror_view_idx>` index numbers each blindspot mirror, counting from right to left in positive y-direction, and from the front to the rear, starting with 0.
...
This group contains all parts of the main moving part of the mirror on the vehicle's left side, which adjusts when moved.
It is a child of the corresponding mirror mounting.

The `<side_mirror_joint_left_idx>` index numbers each side mirror on the left side, counting from right to left in positive y-direction, and from the front to the rear, starting with 0.
...
This group contains all parts of the main moving part of the mirror on the vehicle's right side, which adjusts when moved.
It is a child of the corresponding mirror mounting.

The `<side_mirror_joint_right_idx>` index numbers each side mirror on the right side, counting from right to left in positive y-direction, and from the front to the rear, starting with 0.
...
The `<side_mirror_mounting_left_idx>` index numbers each side mirror on the left side, counting from right to left in positive y-direction, and from the front to the rear, starting with 0.
...
The `<side_mirror_mounting_right_idx>` index numbers each side mirror on the right side, counting from right to left in positive y-direction, and from the front to the rear, starting with 0.
...
The `<side_mirror_view_left_idx>` index numbers each mirrors on the left side, counting from right to left in positive y-direction, and from the front to the rear, starting with 0.
...
The `<side_mirror_view_right_idx>` index numbers each mirrors on the right side, counting from right to left in positive y-direction, and from the front to the rear, starting with 0.
...
`<axle_idx>` denotes the index of the axle to which the wheel is mounted, counting from the front to the rear, starting at 0.
`<wheel_idx>` denotes the index of the wheel on the specified axle, counting from right to left in the positive y-direction, starting at 0.
...
Zero rotation and and translation around all axles are defined under neutral load conditions.
...
They do not change their state during the simulation, which is why lights are considered dynamic.
...
They may also change their state during the simulation; for example, lights are considered dynamic.
...
The `<eyepoint_idx>` index numbers all eye points, counting from right to left in positive y-direction, and from the front to the rear, starting with 0.
...
This group contains all parts of the main moving part of the rearview mirror, which adjusts when moved.
It is a child of the corresponding mirror mounting.

The `<rearview_mirror_joint_idx>` index numbers all rearview mirror joints, counting from right to left in positive y-direction, and from the front to the rear, starting with 0.
...
The `<rearview_mirror_mounting_idx>` index numbers all rearview mirror mountings, counting from right to left in positive y-direction, and from the front to the rear, starting with 0.
...
The `<rearview_mirror_view_idx>` index numbers all rearview mirrors, counting from right to left in positive y-direction, and from the front to the rear, starting with 0.
...
The `<seat_idx>` index numbers all seats, counting from right to left in positive y-direction, starting with 0.
...
A vehicle part is a large component of the vehicle that moves based on other parts of the same vehicle.
