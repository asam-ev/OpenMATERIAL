...
surface. This excludes physical material properties. For a combination of 3D
...
co-simulation platform handles the timing and data transfer between the
individual models.
...
Material assignment::
A mapping of material IDs to parts of 3D models. The material ID is a unique
...
Material property::
Denotes the physical, chemical, and geometric properties of a material.

Multispectral/hyperspectral material::
Material type that interacts with light across multiple wavelengths.
...
Physical property::
Denotes a characteristic of an object that can be measured.
...
co-simulation platform orchestrating a co-simulation of multiple sensors,
models, vehicle dynamics models, and so on.
