...
* The object's geometry shall be in real word scale, using meters as measurement for distances.
* The object's origins and pivot points shall coincide with the origin of the reference coordinate frame (0/0/0) if not specified otherwise.
...
* Meshes shall be triangulated and potential normal maps shall fit to that triangulation.
* Rendered materials shall use the PBR workflow and there shall not be geometry without assigned material.
* Additional requirements apply, when OpenMATERIAL 3D assignment textures are used:
* UVs shall not overlap, they shall be within 0-1 UV space and channel 1 shall be used for assignment textures.
* UV Islands have large enough shall have sufficently big margins inbetween, so that assignment texture interpolation errors are avoided.
...
* The lowest number of polygons possible should be strived for while still retaining the model’s shape.
* The texel density should be homogeneous and as low as possible.
* UV stretching should be minimized.
* Naming of files, nodes, meshes and materials should be meaningful.
* The usage of multiple PBR maps is encouraged (albedo/roughness/metallic/normals).
