...
{THIS_STANDARD} advances standardization by providing standardized definitions of material properties and 3D geometry.
...
{THIS_STANDARD} supports a modular approach to compliance. Based on their individual use-cases, users of {THIS_STANDARD} can decide if they want to implement the standard for 3D models, materials, or both:
...
* *Full Compliance*: For a full exchange and use of simulation-ready 3D assets however, both parts must be supported. Users who implement and support both material and geometric data according to this standard can claim full compliance with {THIS_STANDARD}.
...
{THIS_STANDARD} defines various file formats for specifying assets, material properties, and the mapping between them.
...
The first interaction with an {THIS_STANDARD}-compliant 3D model is through the asset file.
...
* Mapping a texture code of an {THIS_STANDARD} assignment texture to a material property file, enabling detailed differentiation between material properties.
