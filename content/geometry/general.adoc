...
.Transformation of asset coordinates to ASAM OpenSCENARIO and ASAM OSI
[cols="1,1,1"]
|===
|Standard |Coordinate System | Transformation

|ASAM OpenSCNEARIO XML | Vehicle | The origin is the center of the rear axle projected to the ground. Add the x-coordinate of the xref:../geometry/asset-schema.adoc#_rearaxle[rear axle] from the vehicle asset metadata to the {THIS_STANDARD} reference frame.
|ASAM OSI |Moving or stationary object | The origin is in the center of the bounding box. Add half of the height of the asset defined as the maximum z-coordinate in the xref:../geometry/asset-schema.adoc#_boundingbox[bounding box] to the {THIS_STANDARD} reference frame.
|ASAM OSI |Host vehicle | The origin is the center of the rear axle. Add the x- and z-coordinate of the xref:../geometry/asset-schema.adoc#_rearaxle[rear axle] from the vehicle asset metadata to the {THIS_STANDARD} reference frame.
|===
...
The user is free to add more keywords for themself, if they are needed.

* All components shall be named according to capital Snake_Case definition, starting with uppercase letters.
...
* Naming of fields shall be according to CamelCase definition, starting with lowercase letters.
